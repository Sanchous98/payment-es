...
use PaymentSystem\Events\DisputeExpired;
...
class DisputeAggregateRoot implements AggregateRootWithSnapshotting
{
    ...
    public function create(CreateDisputeCommandInterface $command): static
    {
        ...
        $this->recordThat(new DisputeCreated(
    ...
    public function expired(): static
    {
        ...
        $this->recordThat(new DisputeExpired());
    ...
    protected function applyDisputeExpired(): void
    {
        $this->status = DisputeStatusEnum::EXPIRED;
    }
    ...
    protected function apply(DisputeCreated|DisputeExpired|DisputeLost|DisputeWon $event): void
    {
        match ($event::class) {
            DisputeCreated::class => $this->applyDisputeCreated($event),
            DisputeExpired::class => $this->applyDisputeExpired(),
            DisputeLost::class => $this->applyDisputeLost(),
            DisputeWon::class => $this->applyDisputeWon(),
        };
    ...
    protected function applySnapshot(Snapshot $snapshot): void
    {
        ...
        $this->fee = new Money($snapshot->state()['fee']['amount'], new Currency($snapshot->state()['fee']['currency']));
