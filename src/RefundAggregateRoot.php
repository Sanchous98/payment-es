...
class RefundAggregateRoot implements AggregateRootWithSnapshotting
{
    ...
    public static function create(CreateRefundCommandInterface $command): self
    {
        ...
        $command->getPaymentIntent()->is(
            PaymentIntentStatusEnum::SUCCEEDED
        ) || throw RefundUnavailableException::unsupportedIntentStatus($command->getPaymentIntent()->getStatus());
        $command->getPaymentIntent()->getMoney()->lessThan(
            $command->getMoney()
    ...
    protected function apply(object $event): void
    {
        switch ($event::class) {
            ...
            case RefundDeclined::class:
                ...
                break;
            case RefundSucceeded::class:
                $this->applyRefundSucceeded();
                break;
        };
