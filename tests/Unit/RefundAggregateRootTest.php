...
test('payment can be refunded multiple times', function (PaymentIntentAggregateRoot $paymentIntent) {
    ...
    $firstRefund = RefundAggregateRoot::reconstituteFromEvents(new IntId(1), generator());
    $firstRefund->create($command);

    $secondRefund = RefundAggregateRoot::reconstituteFromEvents(new IntId(1), generator());
    $secondRefund->create($command);
